...
server <- function(input, output) {
  ...
  data2 <- read.csv("4 star hotel in barcelona.csv")
  ...
  # Ratings  value should be positive
    ...
    # hotelName <- unlist(str_split(input$hotelName, "in"))
    targetHotel <- "Hilton Barcelona"
    
    targetHotelRatings  <- 4.3
    targetHotelReviews <- 3100
    
    # selected columns
    data <- data2 %>% filter(tolower(targetHotel) != tolower(HotelName)) %>% select(c(HotelName, Ratings, Prices, Reviews, RoomType))
    # seperate lists based on RoomType
    data$RoomType <- factor(data$RoomType, levels = unique(data$RoomType))
    hotelInfo <- split(data, data$RoomType)
    
    filteredHotel1 <- lapply(hotelInfo, function(hotel){
      return(hotel %>% filter(Ratings >= targetHotelRatings - 0.3, Ratings <= targetHotelRatings + 0.3))
    })
    filteredHotel2 <- lapply(hotelInfo, function(hotel){
      return(hotel %>% filter(Ratings >= targetHotelRatings - 0.5, Ratings <= targetHotelRatings + 0.5))
    })
    filteredHotel3 <- lapply(hotelInfo, function(hotel){
      return(hotel %>% filter(Ratings >= targetHotelRatings - 0.7, Ratings <= targetHotelRatings + 0.7))
    })
    filteredHotel4 <- lapply(hotelInfo, function(hotel){
      return(hotel %>% filter(Ratings >= targetHotelRatings - 0.9, Ratings <= targetHotelRatings + 0.9))
    })
    
    filteredHotelInfo <- NULL
    
    for(room in roomType){
      if(nrow(filteredHotel1[[room]]) >= 4){
        filteredHotelInfo[[room]] <- filteredHotel1[[room]]
        next
      }else if(nrow(filteredHotel2[[room]]) >= 4){
        filteredHotelInfo[[room]] <- filteredHotel2[[room]]
        next
      }else if(nrow(filteredHotel3[[room]]) >= 4){
        filteredHotelInfo[[room]] <- filteredHotel3[[room]]
        next
      }else if(nrow(filteredHotel4[[room]]) >= 4){
        filteredHotelInfo[[room]] <- filteredHotel4[[room]]
        next
      }else{
    ...
    generatePlot <- function(targetHotel, hotelInfo) {
      ...
      firstThreeWords <- sapply(str_split(hotelInfo$HotelName, "\\s+"), function(words) paste(words[1:min(length(words),3)], collapse = " "))
      # nodes
      ...
      ggraph(graph, layout = layout) +
        ...
        geom_text(aes(x = layout[, 1], y = layout[, 2], label = str_wrap(nodes$label, width = 13)),
                  ...
                  size = ifelse(nodes$id == rowIndexOfTargetHotel, ifelse(length(hotelInfo$HotelName) == 2, 5, 8),
        ...
        if(length(hotelInfo[[rowIndex]]$HotelName) > 1){
          fluidRow(
            ...
            column(
              ...
              h3(paste0(names(hotelInfo)[rowIndex], " Room"," Prices (Date - ", input$selected_date, ")"), align = "center", style = "margin-bottom: 40px; margin-top: 50px"),
